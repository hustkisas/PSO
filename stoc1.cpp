...
double StochasticLib1::NormalTruncMy(double m, double s, double lb, double ub){
   // Truncated normal distribution
   // The tails are cut off so that the output
   // is in the interval from (m-limit) to (m+limit)
   //if ((m-lb) < s || (ub - m) < s) FatalError("limit out of range in NormalTrunc function");
   double x;
   do {
      x = Normal(0., s);
   } while (  (x + m ) < lb ||(x + m ) > ub ); // reject if beyond limit
   return x + m;
}
